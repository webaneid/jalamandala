...
// CTA BANNER
// ─────────────────────────────────────────────
function WaRotatorButton({ label, source }: { label: string; source: string }) {
  return (
    <a
      href={`/api/wa-rotator/redirect?source=${source}`}
      className="inline-flex h-11 items-center gap-2.5 rounded-2xl bg-[#25d366] px-8 text-sm font-semibold text-white shadow-[0_6px_20px_rgba(37,211,102,0.30)] transition hover:brightness-110 active:scale-95"
    >
...
export function CtaBannerBlock({ payload }: { payload: any }) {
  ...
  const ctaLink = p?.cta_link ?? p?.buttonHref ?? null;
  const waLabel = p?.wa_label ?? "Hubungi Kami via WhatsApp";
  if (!title) throw new Error("CtaBannerBlock: missing title");

  const hasCustomCta = ctaText && ctaLink;
          ...
          <div className="relative z-10 space-y-3">
            ...
            <div className="pt-2">
              {hasCustomCta ? (
                ...
                <WaRotatorButton label={waLabel} source="cta_banner" />
              )}
            </div>
          </div>
        </div>
      ...
      <DarkCard className="text-center">
        ...
        <div className="mt-6">
          {hasCustomCta ? (
            ...
            <WaRotatorButton label={waLabel} source="tenant_cta" />
          )}
        </div>
      </DarkCard>
    </Section>
